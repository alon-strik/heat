...
class FGT(object):
    ...
    def login(self, name, key):
        ...
        res = self.session.post(url, data='username=' + name + '&secretkey=' + key)
                                #verify = True)
...
#    fgt.get('/api/v2/cmdb')
    fgt.get('/api/v2/monitor/system/interface?interface_name=port1', params={"action": "schema", "vdom": vdom})
    """
    fgt.get('/api/v2/monitor/system/interface', params={"action": "schema", "vdom": vdom})
